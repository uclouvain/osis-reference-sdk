...
# **high_school_read**
> HighSchool high_school_read(uuid)
...
with osis_reference_sdk.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.high_schools_list(search=search, country=country, name=name, acronym=acronym, zipcode=zipcode, type=type, accept_language=accept_language, x_user_first_name=x_user_first_name, x_user_last_name=x_user_last_name, x_user_email=x_user_email, x_user_global_id=x_user_global_id, limit=limit, offset=offset)
        pprint(api_response)
    except osis_reference_sdk.ApiException as e:
